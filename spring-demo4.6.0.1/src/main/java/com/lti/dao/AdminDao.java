...
public class AdminDao {
	...
	public void saveCustomerDataIntoAdminDatabase(Customer customer, Admin admin) {
		...
		System.out.println("is this the second saves?");
		 entityManager.merge(admin);  // both insert and update
	}
	/*
	 * public void fetchDataFromCustomerObject(Customer customer,Admin admin) { int
	 * aadhar = customer.getAadharno(); Query q = entityManager.
	 * createQuery("select accno from Customer c  where c.aadharno = :aadhar");
	 * q.setParameter("aadhar", aadhar); Customer custaccno = (Customer)
	 * q.getSingleResult(); admin.setEmailid(customer.getEmailid()); }
	 */
	...
	public List<String> getEmail() {
		 ...
		 * 
		 * while (iterator.hasNext()) { System.out.println("Value is : "+
		 * iterator.next() ); } System.out.println("leaving admin dao getEmail()");
		 */
