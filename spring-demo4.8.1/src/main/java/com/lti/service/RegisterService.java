...
public class RegisterService 
{
	...
	public void fetchPasswordFromSetPasswordPage( Registerib secondreqobj, String loginpass , String transpass )
	{
	    ...
	    email = registerDao.getEmail( secondreqobj );
	 ...
	 */
		...
		public String otpEnteredByUser( Registerib thirdreqobj, Otp otpobj,  int otp )
		{
			...
			if( otpfromthirdrequest == otpgeneratedbyfunction )
			{
				...
				useridaftersaving = registerDao.getUserid();
				sendEmail();
				ret ="userlogin.jsp";
			}
			else
		...
		{
			System.out.println(" inside sendEmail method "+email);
			...
			try {
				...
				message.addRecipient(Message.RecipientType.TO, new InternetAddress(to));
				message.setSubject("Aurora Bank");
				message.setText(" Your Userid is : "+useridaftersaving);
				// send message
				Transport.send(message);
				System.out.println("message sent successfully");
			} 
