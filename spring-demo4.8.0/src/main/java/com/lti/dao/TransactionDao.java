...
public class TransactionDao 
{
	...
	{	
		int transactionamount = transaction.getTransactionamt();
		...
		else
		{
			...
			if ( transactionamount <= availablebalancefromprevioustransaction )
			{
	...
	{
			String q1= "select t.tid from Transaction t where t.tid=:tid";
	...
	{
		String q1= "select t.refbeneficiaryaccno from Transaction t where t.tid=:tid";
		Query q =entityManager.createQuery(q1);	
		q.setParameter("tid", newtid );
		int x = (Integer) q.getSingleResult();
		return x;
	...
	{
		String q1= "select t.balance from Transaction t where t.tid=:tid";
		Query q =entityManager.createQuery(q1);	
		q.setParameter("tid", newtid );
		int x = (Integer) q.getSingleResult();
		return x;
	...
	{
		String q1= "select t.transactionamt from Transaction t where t.tid=:tid";
		Query q =entityManager.createQuery(q1);	
		q.setParameter("tid", newtid );
		int x = (Integer) q.getSingleResult();
		return x;
	...
	{
		String q1= "select t.availablebalance from Transaction t where t.tid=:tid";
		Query q =entityManager.createQuery(q1);	
		q.setParameter("tid", newtid );
		int x = (Integer) q.getSingleResult();
		return x;
