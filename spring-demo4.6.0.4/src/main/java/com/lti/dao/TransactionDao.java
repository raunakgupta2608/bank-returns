...
public class TransactionDao 
{
	...
	{	
		...
		int transactionamt = transaction.getTransactionamt();
			...
			else
			{
				...
				{
					setAvailableBalanceToBalance(userid,transaction);
					doTransaction( transaction, userid );
					ret = "dashboard.jsp";
				}
				else
	...
	@Transactional
	public void setAvailableBalanceToBalance( int refuserid, Transaction transaction )  throws NoResultException
	{
		try
		{
			System.out.println("entered dao setAvailableBalanceToBalance");
			...
			List balanceafterupdate = q1.getResultList();
			ListIterator iterator = balanceafterupdate.listIterator();
			
			 while (iterator.hasNext()) 
			 { 
				 finalbalanceafterupdate = (Integer) iterator.next();
			 }
			 System.out.println(finalbalanceafterupdate);
			 System.out.println("leaving setAvailableBalanceToBalance");
			
			transaction.setBalance(finalbalanceafterupdate);
		}
		catch(NoResultException nre)
		{ System.out.println(nre); }
	}
